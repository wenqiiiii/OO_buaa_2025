    ...
    public PolyExpr ToPoly() {  //将factors容器中的各因子相乘
        ...
        for (Factor fac : factors) {
            if (fac instanceof NumFac) {
                ...
                PolyExpr facPoly = null;
                if (fac instanceof ExprFac) {        //乘表达式因子
                    facPoly = ((ExprFac) fac).ToPoly();
                } else if (fac instanceof RecurFac) {
                    facPoly = ((RecurFac) fac).ToPoly();
                } else if (fac instanceof NormalFac) {
                    facPoly = ((NormalFac) fac).ToPoly();
                } else if (fac instanceof DeriveFac) {
                    facPoly = ((DeriveFac) fac).ToPoly();
                    if (facPoly.IsEmpty()) {
                        facPoly.putPowerFunc(new BigInteger("0"), new BigInteger("1"));
                    }
                }
                if (poly.IsEmpty()) {
