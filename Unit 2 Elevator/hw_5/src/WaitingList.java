...
public class WaitingList {     
    ...
    public synchronized Boolean hasSameDir(int floor, String dir) {
        ...
        return seekSameDir(floor, dir);
    }

    private Boolean seekSameDir(int floor, String dir) {
        Iterator<PersonRequest> iter = personRequests.iterator();
        while (iter.hasNext()) {
            PersonRequest pr = iter.next();
            if (floorToInt(pr.getFromFloor()) > floor && dir.equals("UP")) {
                return true;
            } else if (floorToInt(pr.getFromFloor()) < floor && dir.equals("DOWN")) {
                return true;
            }
        }
        return false;
    ...
    public synchronized PersonRequest findPickup(int floor, String direction) {
        ...
        Iterator iter = personRequests.iterator();
        while (iter.hasNext()) {
            PersonRequest now = (PersonRequest) iter.next();
    ...
    private Boolean canPickup(int floor, String direction, PersonRequest one) {
        //策略是携带from相同且同向
        int from = floorToInt(one.getFromFloor());
        if (from == floor) {
            int to = floorToInt(one.getToFloor());
    ...
    private int floorToInt(String f) {
